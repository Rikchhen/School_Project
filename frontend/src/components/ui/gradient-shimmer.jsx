...
/*
 * GradientShimmer — sweeps a multi-stop gradient highlight across plain text.
 * Web-Animations-API driven, zero runtime dependencies, no CSS import.
 *
 * Ported to JSX (this app is Vite + JavaScript + styled-components, not
 * shadcn/Tailwind/TypeScript). The component is self-contained and needs
 * neither Tailwind nor shadcn — only its TypeScript types were stripped.
 *
 * A single stop in the highlight band is `{ position: 0..1, color }`.
 * `gradient` accepts either an array of those stops or a preset name below.
 */
 ...
 *
 * Props:
 *   children             string to shimmer (plain text only)
 *   gradient             stop array or preset name (default "sunrise")
 *   easing               "smooth" | "gentle" | "snappy" (default "smooth")
 *   duration             sweep time in seconds (default 1.45)
 *   spread               highlight band width in px per character (default 3)
 *   angle                gradient angle in degrees (default 105)
 *   pauseBetween         idle gap in ms between sweeps (default 1000)
 *   baseColor            color the band fades into (default "currentColor")
 *   pauseOnScroll        pause while the page scrolls (default true)
 *   pauseWhenOffscreen   pause while offscreen (default true)
 *   respectReducedMotion static gradient under reduced motion (default true)
